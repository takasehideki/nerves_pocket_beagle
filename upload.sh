...
#
# If unspecifed, the destination is nerves_pocket.local and the .fw file is naively
# guessed
#
...
help() {
    ...
    echo
    echo "Default destination IP is 'nerves_pocket.local'"
...
[ -n "$DESTINATION" ] || DESTINATION=nerves_pocket.local
[ -n "$MIX_TARGET" ] || MIX_TARGET=bbb
